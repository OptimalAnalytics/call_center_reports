...
all_act = pd.concat(
    [buckets['Acct_Num'], rpc['Acct Id Acc']], ignore_index=True)
...
agents = pd.concat(
    [buckets['Associate'], rpc['Created By Qcc']], ignore_index=True)
...
agents['Agents_new'] = agents.apply(
    lambda x: '{}-{}'.format(x['Agents'][:2], x['Agents_new']), axis=1)
